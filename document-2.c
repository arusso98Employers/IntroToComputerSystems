...
int print_document(Document *doc){
  ...
  int i = 0, j  = 0, p = doc->number_of_paragraphs -1;  
	...
	/*if i is not the final paragraph and there are valid lines,
	  then there is a paragraph separation*/
...
int get_number_lines_paragraph(Document *doc, int paragraph_number,
  ...
  if(doc!= NULL &&
    ...
    /*extracting information from the document/paragraph structure*/
    *number_of_lines =
      doc->paragraphs[paragraph_number -1].number_of_lines;
...
int append_line(Document *doc, int paragraph_number, const char *new_line){
   ...
   if(doc != NULL && paragraph_number <= doc->number_of_paragraphs
     ...
     /*every appended lines causes the overall number of lines 
       to increase*/
     doc->paragraphs[paragraph_number -1].number_of_lines ++;
...
int remove_line(Document *doc, int paragraph_number, int line_number){
  ...
  if(doc != NULL && doc->number_of_paragraphs >= paragraph_number
    ...
    /*moves all the index's over starting at the line to remove*/
    ...
    /*line number decreases if a line is removed*/
    doc->paragraphs[paragraph_number -1].number_of_lines --;
    return SUCCESS;
...
int load_document(Document *doc, char data[][MAX_STR_SIZE + 1],
  ...
  if((doc == NULL) || (data == NULL) || (data_lines == 0)){
    ...
    int i = 0, count = 0, para = 1;
    add_paragraph_after(doc, 0);
    
    for(i = 0; i < data_lines; i++){
      if(strcmp(data[i], "") == 0){
	count ++;
	add_paragraph_after(doc, count);
	para ++;
      }else{
	append_line(doc, para, data[i]);
...
int replace_text(Document *doc, const char *target,
  ...
  if(doc != NULL && target != NULL && replacement != 0){
	...
	for(j = 0; j < doc->paragraphs[i].number_of_lines; j++){
	  ...
	  /*p points to the first element in the current line*/
	  ...
	  /*set the null character at the end of the array*/
	  replace_array[index] = '\0';

	  /*replace_array now contains a new line, which replaces the 
	    current line in the document/paragraph structure*/
...
int highlight_text(Document *doc, const char *target){
  ...
  /*creating the replacement variable which is simply the target 
    with the highlight prefix and suffix*/
  ...
  if(doc != NULL && target != NULL){
	...
	/*sets the null character at the end of the array*/  
	replace_array[index] = '\0';
...
int remove_text(Document *doc, const char *target){
  ...
  /*replacement is set to the empty character*/
